...
auto invoke_from_cbor(
    ...
    if constexpr (std::is_void_v<R>) {
        ...
        return cbor_encode(
            ::nlohmann::json(fn(args.at(I).template get<std::decay_t<A>>()...))
        );
    }
}
 ...
 * 等任何 nlohmann/json 能序列化的类型.  `std::string` 参数也允许携带任意
 * 字节 (含嵌入 NUL), 不再要求 UTF-8.
 *
...
auto call(
    ...
    (args_cbor_value.push_back(::nlohmann::json(std::move(args))), ...);
    ...
    if constexpr (std::is_void_v<Ret>) {
        ...
        return detail::cbor_decode(std::move(response)).template get<Ret>();
    }
}
