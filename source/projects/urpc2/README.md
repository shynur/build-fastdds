...
仅当环境变量 `RBK_IN_CAR` 存在且其值非空字符串时, 通信被限制在 `192.168.192.0/24`;
否则沿用 Fast DDS 默认的 builtin transport (所有网卡).  网段本身写死在代码里, 不
可配置; `RBK_IN_CAR` 只是开关.
...
实现方式见 `src/urpc2.cpp` 的 `create_participant()`: 开启时关闭 builtin transport,
改用一个自建的 UDPv4 transport, 其 interface allowlist 填入本机在该网段上的地址.
allowlist 同时约束收和发, 因此用户数据与 builtin 发现报文 (含多播) 都只经过该
网卡; 同时打开 `ignore_non_matching_locators` 丢弃网段外对端宣告的 locator.

车内 UDP transport 的 `maxMessageSize` 为 1400 字节.  该值限制的是单个 UDP
datagram, 不限制 `Urpc2::call()` 的 payload: Fast DDS 会透明地把任意大小的 DDS
sample 拆成 MTU 内的 RTPS fragments, 接收端再透明重组.  这样避免默认约 64 KiB
datagram 在 1500 MTU Ethernet 上产生 IP 分片, 也让有限的 socket receive buffer
能容纳更多待处理 fragment.  transport 同时请求 1 MiB 的 send/receive socket
buffer; 实际值仍受各主机 `net.core.wmem_max` / `net.core.rmem_max` 上限约束.
...
测试环境不设置 `RBK_IN_CAR`, 因此不受上述网段限制, CI 与本地容器都能正常跑.
