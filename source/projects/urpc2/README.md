...
通信被写死限制在 `192.168.192.0/24`, 不可配置 (没有开关, 不读环境变量或 XML).
这对应固定的部署形态: 库运行在小车内的三个控制器上, 控制器之间由手工组成的
`192.168.192.0/24` 局域网互联.

实现方式见 `src/urpc2.cpp` 的 `create_participant()`: 关闭 builtin transport,
...
CI (`.github/workflows/build.yaml` 经 `scripts/ci-build.bash`) 里的测试流程已禁用:
`run-tests.bash` 那行被注释掉了, CI 现在只构建, 打包和发布.  原因是上述网段限制,
CI runner 上没有 `192.168.192.*` 网卡, 相关用例必然失败.

下面的手工测试流程需要一个具备该网段的环境 (例如给容器接入相应网络) 才能跑通.
