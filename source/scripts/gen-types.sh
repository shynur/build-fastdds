#!/usr/bin/env bash
# scripts/gen-types.sh - 用 fastddsgen 为各测试用例的 IDL 生成类型/RPC 支持代码,
# 就地输出到 <test>/src/types/ (这些生成物不入库, 仅在此步产生).
#
# 前提: fastddsgen 已在 PATH 中 (见 build-fastddsgen.sh), 或用 FASTDDSGEN 指定其路径.
...
command -v "${GEN}" >/dev/null 2>&1 \
    || { echo "找不到 fastddsgen; 请先运行 build-fastddsgen.sh 并将其 scripts/ 加入 PATH" >&2; exit 1; }
