...
class ProcessorServerLogic
    : public frpc::RpcServer
    , public std::enable_shared_from_this<ProcessorServerLogic>
{
...
private:
    ...
    struct RequestContext : frpc::RpcRequest
    {
        ...
        {
            size_t n_fields = 0;
            n_fields += request.router.has_value() ? 1 : 0;
        ...
        bool prepare(
                frpc::Replier* replier)
        {
            if (request.router.has_value())
            {
                return prepare_router(replier);
            }
            ...
            const std::shared_ptr<RequestContext>& req)
    {
        try
        {
            for (;;)
            {
                if (req->request.router.has_value())
                {
                    {
                        ReplyType reply{};
                        reply.router = urpc2::gen::detail::Processor_router_Result{};
                        reply.router->result = urpc2::gen::detail::Processor_router_Out{};
                        reply.router->result->return_ = implementation_->router(
                            *req,
                            req->request.router->handler_name,
                            req->request.router->args);
    ...
    std::shared_ptr<ProcessorServer_IServerImplementation> implementation_;

};

struct ProcessorServerProxy
    : public frpc::RpcServer
{
    ProcessorServerProxy(
            std::shared_ptr<frpc::RpcServer> impl)
        : impl_(std::move(impl))
    {
    }

    ~ProcessorServerProxy() override
    {
        if (impl_)
        {
            impl_->stop();
...
std::shared_ptr<eprosima::fastdds::dds::rpc::RpcServer> create_ProcessorServer(
        ...
        std::shared_ptr<ProcessorServer_IServerImplementation> implementation)
{
    auto ptr = std::make_shared<detail::ProcessorServerLogic>(
        part, service_name, qos, thread_pool_size, implementation);
    return std::make_shared<detail::ProcessorServerProxy>(ptr);
}
