...
> 依赖 `foonathan_memory` 则**不再**作为 submodule: 其所需版本已由 Fast DDS 自己声明, 改由
> `scripts/fetch-foonathan.bash` 于 CI 期动态 clone, 随 Fast DDS 升级自动跟随.
...
测试后由 `scripts/package-deb.bash` 打成 `urpc2_<版本>_<amd64|arm64>.deb` (安装前缀 `/usr/local`)
